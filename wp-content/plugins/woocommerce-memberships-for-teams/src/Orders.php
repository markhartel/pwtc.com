...
class Orders {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_order_status_completed',  array( $this, 'create_teams_from_order' ), 10 );
		add_action( 'woocommerce_order_status_processing', array( $this, 'create_teams_from_order' ), 10 );
	...
	public function create_teams_from_order( $order ) {
		...
		// loop over all items in cart, and one-by-one, create a team for any applicable items - note that this allows creating multiple teams
		// for the same membership plan
		foreach ( $order_items as $item_id => $item ) {
			...
			if ( $product && Product::has_team_membership( $product ) ) {
				...
				if ( $team_id ) {
					...
					// do not allow the same order to renew or reactivate the team membership: this prevents admins changing order statuses from extending/reactivating the team membership
					// also, bail out if the team plan cannot be found
					if ( ! $team instanceof Team || $team->get_order_id() === $order->get_id() || ! ( $plan = $team->get_plan() ) ) {
						return;
					}
				...
				try {
					...
					$team = wc_memberships_for_teams_create_team( array(
						'team_id'    => $team_id,
						'product_id' => $product->get_id(),
						'order_id'   => $order->get_id(),
						'owner_id'   => $user_id,
						'name'       => $item->get_meta( 'team_name' ),
						'seats'      => $seats,
					), $action );

					wc_update_order_item_meta( $item_id, '_wc_memberships_for_teams_team_id', $team->get_id() );
					 ...
					 * @since 1.0.0
					 *
					 * @param \SkyVerge\WooCommerce\Memberships\Teams\Team $team the team that was created
					 * @param array $args contextual arguments
					 */
					...
					/* translators: %1$s - order number, %2$s - order item id, %3$s - error message */
					wc_memberships_for_teams()->log( sprintf( __( 'Could not create team from order %1$s item %2$s: %3$s', 'woocommerce-memberships-for-teams' ), $order->get_order_number(), $item->get_id(), $e->getMessage() ) );
				}
	 ...
	 * @param \WC_Order_Item|null $item the order item
	...
	public function format_team_meta_for_display( $formatted_meta, $item ) {
		...
		foreach ( $formatted_meta as $meta_id => $meta ) {
			...
			if ( ! \SV_WC_Helper::str_starts_with( $meta->key, 'team_' ) ) {
				continue;
			}
			...
			if ( $team instanceof Team ) {
				...
				$display_value = wpautop( make_clickable( apply_filters( 'woocommerce_order_item_display_meta_value', sprintf( '%s %s', $meta->value, $team_link ), $meta, $item ) ) );
		...
		foreach ( $refund->get_items() as $item ) {
			...
			if  ( $parsed_item['has_partial_quantity'] || $parsed_item['has_partial_total'] ) {
				...
				if ( $invalid ) {
					...
					if ( \SV_WC_Plugin_Compatibility::is_wc_version_lt( '3.3' ) ) {
	...
	private function parse_refund_item( $item, $team_ids, $order ) {
		...
		$team_id          = $refunded_item_id ? wc_get_order_item_meta( $refunded_item_id, '_wc_memberships_for_teams_team_id', true ) : null;
