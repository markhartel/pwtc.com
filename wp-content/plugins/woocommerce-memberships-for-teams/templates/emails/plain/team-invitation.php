...
if ( $invitation->get_user() ) {
	...
	if ( $existing_membership = $invitation->get_recipient_existing_membership() ) {
		...
		if ( $current_team = wc_memberships_for_teams()->get_teams_handler_instance()->get_user_membership_team( $existing_membership->get_id() ) ) {
			printf( esc_html__( 'You are a member of %1$s, which already gives you access to %2$s. Joining %3$s means you will leave your current team and your existing membership will be moved under new team management.' ), $current_team->get_name(), $team->get_plan()->get_name(), $team->get_name() );
		} else {
