...
if (!function_exists('wp_rml_move')) {
    ...
    function wp_rml_move($to, $ids) {
        ...
        if ($folder !== null) {
            do_action("RML/Item/Move", $folder, $ids);
            $folder->moveItemsHere($ids);
            do_action("RML/Item/MoveFinished", $folder, $ids);
        }else{
            do_action("RML/Item/Move", -1, $ids);
            if (is_array($ids) && count($ids) > 0) {
                foreach ($ids as $value) {
                    ...
                    //do_action("RML/Item/Moved", $value, null); @deprecated, @see RML_Filter::update_postmeta
                }
            }
            do_action("RML/Item/MoveFinished", -1, $ids);
        }
    }
}
