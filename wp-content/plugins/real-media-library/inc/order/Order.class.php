...
class RML_Order {
    ...
    public function index($id, $delete = true) {
        ...
        $sql = $wpdb->prepare("INSERT INTO " . $this->table_name . " (`attachment`, `fid`, `nr`) 
                SELECT
    ...
    public function posts_clauses($pieces, $query) {
        ...
        if (!empty($query->query_vars['parsed_rml_folder']) &&
            ...
            if (wp_rml_is_type($folder, array(RML_TYPE_GALLERY))) {
                ...
                // Index
                /*
                $index = $this->needsIndex($folder);
                if ($index[1]["cnt"] <= 0 && $index[0]["cnt"] > 0) {
                    $this->index($folder, false);
                }
                */
     ...
     */
    public function item_move($folder, $ids) {
        /*if ($folder instanceof RML_Folder && wp_rml_is_type($folder, array(RML_TYPE_GALLERY))) {
            //$this->needsIndex[$folder->id] = $this->needsIndex($folder->id);
            $needsIndex = $this->needsIndex($folder->id);
            if ($needsIndex[1]["cnt"] <= 0 && $needsIndex[0]["cnt"] > 0) {
                $this->index($folder->id, false);
            }
        }*/
        ...
        if ($folder instanceof RML_Folder) {
            ...
            if ($needsIndex[1]["cnt"] <= 0) {
                $this->debug_sqls("Avoid push process for " . $folder->id);
                $this->avoidFids[] = $folder->id;
            }
        }
     ...
     */
    public function item_move_finished($folder, $ids) {
        // Index if needed
        /* if ($folder instanceof RML_Folder && wp_rml_is_type($folder, array(RML_TYPE_GALLERY))) {
            if (isset($this->needsIndex[$folder->id]) && $this->needsIndex[$folder->id][1]["cnt"] <= 0) {
                $this->index($folder->id, false);
            }
        }*/
    ...
    private function items_push($pushTo) {
        ...
        foreach ($pushTo as $val) {
            ...
            if ($val["insert"] == true) {
                // Is insert
                $inserts[] = "(" . $val["attachmentId"] . ", " . $val["toFid"] . ", " . $i . ")";
            }else{
                // Is Update
        ...
        if ($folderId !== null & $cntToPush > 0) {
            ...
            // Are inserts needed
            if (count($inserts) > 0) {
                $sql_insert = "INSERT INTO " . $this->table_name . " (`attachment`, `fid`, `nr`) VALUES " . implode(",", $inserts);
