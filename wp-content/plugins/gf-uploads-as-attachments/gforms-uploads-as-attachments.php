...
function jbl_gfuaa_load_text_domain() {
    load_plugin_textdomain( 'gf-uploads-as-attachments', FALSE, basename( dirname( __FILE__ ) ) . '/languages/' );
}
add_action( 'plugins_loaded', 'jbl_gfuaa_load_text_domain' );
...
function jbl_add_gform_notification_settings( $ui_settings, $confirmation, $form ) {
	...
	$ui_settings['jbl_uaa_settings'] = '
		<tr valign="top">
			<th colspan=2>
				<hr>
				<strong>' . __( 'Uploads as Attachments settings', 'gf-uploads-as-attachments' ) . ': </strong>
			</th>
		</tr>
		<tr valign="top">
			<th scope="row">
				<label for="jbl_gfuaa_enable">' . __( 'Enable', 'gf-uploads-as-attachments' ) . '</label>
			</th>
			<td>
				<input type="checkbox" id="jbl_gfuaa_enable" name="jbl_gfuaa_enable" value="1"' . $checked_enable . '>
				<label for="jbl_gfuaa_enable" class="inline">' . __( 'Add fileupload fields from this form as attachments to this notification', 'gf-uploads-as-attachments' ) . '</label>
				<br>
			</td>
		</tr>
		<tr valign="top">
			<th scope="row">
				<label for="jbl_gfuaa_delete_files_after">' . __( 'Delete files', 'gf-uploads-as-attachments' ) . '</label>
			</th>
			<td>
				<input type="checkbox" id="jbl_gfuaa_delete_files_after" name="jbl_gfuaa_delete_files_after" value="1"' . $checked_delete_files_after . '>
				<label for="jbl_gfuaa_delete_files_after" class="inline">' . __( 'Delete uploaded files from server after notification sent?', 'gf-uploads-as-attachments' ) . '</label>
				<br>
			</td>
		</tr>
	';
