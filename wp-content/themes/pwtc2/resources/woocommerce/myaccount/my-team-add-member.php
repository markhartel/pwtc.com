...
<div class="woocommerce-account-my-teams">
	...
	<p>
		<?php if ( $seat_count > 0 ) : ?>
			<?php printf( _n( 'This family has <strong>%d seat remaining</strong>.', 'This family has <strong>%d seats remaining</strong>.', $remaining_seats, 'woocommerce-memberships-for-teams'  ), $remaining_seats ); ?>
		<?php else : ?>
			<?php _e( 'This family has <strong>unlimited seats</strong>.', 'woocommerce-memberships-for-teams' ); ?>
		<?php endif; ?>
		<?php if ( $team->is_user_owner( get_current_user_id() ) && ! $team->is_user_member( get_current_user_id() ) ) : ?>
			...
			?>
			<?php printf( esc_html__( 'You can %1$sadd yourself as a member%2$s, share your family registration link, or manually add new members below.', 'woocommerce-memberships-for-teams' ), '<a href="' . $action_url . '"><strong>', '</strong></a>' ); ?>
		<?php else: ?>
			<?php esc_html_e( 'You can share your family registration link or manually add new members below.', 'woocommerce-memberships-for-teams' ); ?>
		<?php endif; ?>
	</p>

	<h3><?php esc_html_e( 'Registration Link', 'woocommerce-memberships-for-teams' ); ?></h3>

	<p><?php esc_html_e( 'This registration link will allow members to register themselves for your family. Please use caution when sharing this, as it allows any visitor to add themselves to your family.', 'woocommerce-memberships-for-teams' ); ?></p>
	...
	<?php if ( $seat_count > 0 && ! $remaining_seats ) : ?>
		...
		<p><?php esc_html_e( "You can't add more members because your family has no more seats left.", 'woocommerce-memberships-for-teams' ); ?></p>
	...
	<?php else : ?>
		...
		<p><?php esc_html_e( 'Enter member details - your family member will receive an invitation via email.', 'woocommerce-memberships-for-teams' ); ?></p>
