...
<?php if ( ! $team ) : ?>
	...
	<p class="woocommerce-error"><?php esc_html_e( 'Looks like the link you followed is no longer valid. If you were invited to join a family contact the family owner and ask them for a new invitation.', 'woocommerce-memberships-for-teams' ); ?></p>
...
<?php else : ?>
	...
	<p>
		...
		<?php if ( $invitation ) : ?>
			<?php if ( $current_user && $current_user->user_email === $invitation->get_email() ) : ?>
				<?php esc_html_e( 'Click the button below to join this family.', 'woocommerce-memberships-for-teams' ); ?>
			<?php elseif ( ! $current_user && $invitation->get_user() ) : ?>
				<?php esc_html_e( 'Please log in with your account to join this family.', 'woocommerce-memberships-for-teams' ); ?>
			<?php endif; ?>
		<?php endif; ?>
	</p>
	...
	<?php if ( ( $current_user && ! $invitation || $current_user && $current_user_matches_invitee ) && $existing_user_membership ) : ?>
		<?php if ( $current_team = wc_memberships_for_teams()->get_teams_handler_instance()->get_user_membership_team( $existing_user_membership->get_id() ) ) : ?>
			...
			<p class="woocommerce-info"><?php printf( esc_html__( 'You are a member of %1$s, which already gives you access to %2$s. Joining %3$s means you will leave your current family and your existing membership will be moved under new family management.' ), $current_team->get_name(), $team->get_plan()->get_name(), $team->get_name() ); ?></p>
		<?php else : ?>
			<?php /* translators: %s - membership plan name */ ?>
			<p class="woocommerce-info"><?php printf( esc_html__( 'Your existing %s membership will be moved under family management.' ), $team->get_plan()->get_name() ); ?></p>
		<?php endif; ?>
	<?php endif; ?>
	...
	<?php if ( $current_user ) : ?>
		...
		<?php if ( $team->is_user_member( $current_user->ID ) ) : ?>
			...
			<p class="woocommerce-info"><?php printf( esc_html__( 'You are already a member of this family. %1$sClick here to view your account%2$s.', 'woocommerce-memberships-for-teams' ), '<a href="' . wc_get_page_permalink( 'myaccount' ) . '">', '</a>' ); ?></p>
		...
		<?php else : ?>
			...
			<form id="join-team-form" method="post">
				...
				<?php if ( $invitation && ! $current_user_matches_invitee ) : ?>
					<button class="button" type="submit"><?php /* translators: %s - email */ printf( esc_html__( 'Join Family as %s', 'woocommerce-memberships-for-teams' ), $current_user->user_email ); ?></button>
				...
				<?php else : ?>
					<button class="button" type="submit"><?php esc_html_e( 'Join Family', 'woocommerce-memberships-for-teams' ); ?></button>
				<?php endif; ?>
