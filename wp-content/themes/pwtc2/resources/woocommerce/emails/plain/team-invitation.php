...
printf( esc_html__( 'This will give you %s access.', 'woocommerce-memberships-for-teams' ), $plan->get_name() );
...
if ( $invitation->get_user() ) {
	...
	if ( $existing_membership = $invitation->get_recipient_existing_membership() ) {

		echo "\r\n\r\n";

		if ( $current_team = wc_memberships_for_teams()->get_teams_handler_instance()->get_user_membership_team( $existing_membership->get_id() ) ) {
			printf( esc_html__( 'You are a member of %1$s, which already gives you access to %2$s. Joining %3$s means you will leave your current family and your existing membership will be moved under new family management.' ), $current_team->get_name(), $team->get_plan()->get_name(), $team->get_name() );
		} else {
			printf( esc_html__( 'Your existing %s membership will be moved under team management.', 'woocommerce-memberships-for-teams' ), $plan->get_name() );
		}
	}
